...
mongoose.connect('mongodb://localhost/products',{useNewUrlParser:true,useUnifiedTopology:true}).then(()=>{
...
app.get('/show/:id',function(req,res){
    product.findById(req.params.id).populate("comments").exec(function(err,foundprod){
        if(err){
            console.log(err);
        }
        else{
            
            res.render('product/show',{item:foundprod,currentuser:req.user});
        }
    })
...
app.post('/register',function(req,res){
    ...
    user.register(newUser,req.body.password,function(err,user){
        if(err){
            console.log(err);
            return res.render('products/register');
        }
...
function reduceCart(productid,username){
    var productid = productid;
    user.findOne({username:username},function(err,user){
        if(err)
        console.log(err);
